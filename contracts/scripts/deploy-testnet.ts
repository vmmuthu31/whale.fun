import { parseEther, createWalletClient, http, createPublicClient, getContract } from "viem";
...
async function main() {
  ...
  async function deployContract(name: string, constructorArgs: any[] = [], solFileName?: string) {
    console.log(`\n📝 Deploying ${name}...`);
    
    const solFile = solFileName || name;
    const contractPath = path.join(artifactsPath, `${solFile}.sol/${name}.json`);
  ...
  const tokenFactory = await deployContract("TokenFactory", [whaleToken.address], "TokenFactoryRoot");
  
  // Step 3: Deploy TokenGraduation
  const tokenGraduation = await deployContract("TokenGraduation", [tokenFactory.address]);
  ...
  let hash = await tokenGraduationContract.write.updateEthToUsdRate([initialEthToUsdRate]);
  ...
  hash = await tokenFactoryContract.write.setMinInitialLiquidity([0n]);
  await publicClient.waitForTransactionReceipt({ hash });
  
  hash = await tokenFactoryContract.write.setMaxTokensPerCreator([1000000n]);
  await publicClient.waitForTransactionReceipt({ hash });
  ...
  hash = await tradingEngineContract.write.updateFeeStructure([30, 100, 70, 20, 10]);
  await publicClient.waitForTransactionReceipt({ hash });
