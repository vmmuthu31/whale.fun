...
const MOCK_PROFILES = [
  { name: "Aqua Orca", symbol: "ORCA", description: "Ocean-born memecoin riding tidal hype.", logoUrl: "https://i.imgur.com/3Qv2PjW.png", communitySize: 4200 },
  { name: "Laser Llama", symbol: "LLAMA", description: "Chill vibes. Hot lasers. DeFi herder.", logoUrl: "https://i.imgur.com/9qv2pFQ.png", communitySize: 6800 },
  { name: "Nebula Cat", symbol: "NEKO", description: "Cosmic feline exploring yield galaxies.", logoUrl: "https://i.imgur.com/7nq0s5d.png", communitySize: 12000 },
  { name: "Whale Wink", symbol: "WINK", description: "Big brain whale with diamond fins.", logoUrl: "https://i.imgur.com/2hTzZmi.png", communitySize: 9500 },
  { name: "Pixel Penguin", symbol: "PXP", description: "On-chain tuxedo speedrunner.", logoUrl: "https://i.imgur.com/0qzVg2N.png", communitySize: 5100 },
  { name: "Turbo Toad", symbol: "TOAD", description: "Perpetual hop to ATH.", logoUrl: "https://i.imgur.com/6CwQ9aT.png", communitySize: 7700 },
  { name: "Meta Manta", symbol: "MANTA", description: "Gliding through liquidity streams.", logoUrl: "https://i.imgur.com/t2qB1mE.png", communitySize: 8300 },
  { name: "Zen Zebra", symbol: "ZEBRA", description: "Black & white clarity in markets.", logoUrl: "https://i.imgur.com/3h4rV3H.png", communitySize: 6100 },
  { name: "Rocket Raccoon", symbol: "RKTN", description: "Trash to treasure tokenomics.", logoUrl: "https://i.imgur.com/X9j0e1B.png", communitySize: 10400 },
  { name: "Cyber Shrimp", symbol: "SHRMP", description: "Small bags. Big energy.", logoUrl: "https://i.imgur.com/8D0b7cS.png", communitySize: 5600 },
  { name: "Meme Mako", symbol: "MAKO", description: "Fastest shark on the chain.", logoUrl: "https://i.imgur.com/5dR41Qy.png", communitySize: 9000 },
  { name: "Quantum Quokka", symbol: "QQ", description: "Always smiling. Always compounding.", logoUrl: "https://i.imgur.com/2m3r1hZ.png", communitySize: 4800 },
];
...
async function waitForReceiptRobust(publicClient: ReturnType<typeof createPublicClient>, hash: `0x${string}`) {
  // First try a normal wait with generous timeout
  try {
    return await publicClient.waitForTransactionReceipt({ hash, pollingInterval: 1500, timeout: 90_000 });
  } catch (e) {
...
async function main() {
  ...
  const fundAmountWei = a.fundAmount ? parseEther(String(a.fundAmount)) : undefined; // e.g., --fundAmount 0.1
  ...
  const secondaryList = (process.env.SECONDARY_PRIVATE_KEYS || "").split(/[,\s]+/).filter(Boolean).map((s) => s.replace(/^0x/, ""));
  if (secondaryList.length === 0) throw new Error("SECONDARY_PRIVATE_KEYS required in .env (CSV of private keys)");

  const chain = zeroGTestnet;
  const publicClient = createPublicClient({ chain, transport: http() });
  const rootAccount = privateKeyToAccount(`0x${rootPk}`);
  const rootWallet = createWalletClient({ account: rootAccount, chain, transport: http() });

  const wallets = [rootAccount, ...secondaryList.map((pk) => privateKeyToAccount(`0x${pk}`))];
  const walletClients = new Map<Address, ReturnType<typeof createWalletClient>>();
  walletClients.set(rootAccount.address, createWalletClient({ account: rootAccount, chain, transport: http() }));
  for (const w of wallets.slice(1)) {
    walletClients.set(w.address, createWalletClient({ account: w, chain, transport: http() }));
  }

  console.log(`[rr] Network=${network} Wallets=${wallets.length} actions=${actionsFixed ?? `${actionsMin}-${actionsMax} (random per wallet)`} simulate=${simulate}`);
  if (fundAmountWei) console.log(`[rr] Auto-fund enabled: target per secondary wallet = ${fundAmountWei} wei`);
  ...
  const tradingEngine = getContract({ address: TradingEngineAddr, abi: teArtifact.abi, client: { public: publicClient } });
  ...
  if (fundAmountWei && !simulate) {
    console.log(`[fund] Checking secondary wallets for top-up to ${fundAmountWei} wei`);
    for (const w of wallets.slice(1)) { // exclude root
      ...
      if (bal < fundAmountWei) {
        ...
        const to = w.address;
        const hash = await rootWallet.sendTransaction({ account: rootAccount, chain, to, value: needed });
  ...
  tokensAll = tokensAll.filter((t) => t.toLowerCase() !== (WhaleTokenAddr as string).toLowerCase());
  ...
  if (tokensAll.length === 0) {
    console.log(`[rr] No tokens available to trade. In simulate mode, either pre-create tokens (real run) or ensure factory has tokens.`);
    console.log(`[rr] Exiting.`);
    return;
  }

  // Helper: compute tokenAmount for target ETH (buy)
  async function solveBuyTokenAmount(token: Address, ethBudget: bigint): Promise<{ tokenAmount: bigint; cost: bigint } | null> {
    // Use current price and remaining supply to bound the search space
    const stats = (await publicClient.readContract({ address: token, abi: ctArtifact.abi, functionName: "getTokenStats", args: [] })) as readonly [bigint, bigint, bigint, bigint, bigint, bigint];
    ...
    for (let i = 0; i < 32 && lo <= hi; i++) {
      ...
      try {
        cost = (await publicClient.readContract({ address: token, abi: ctArtifact.abi, functionName: "calculateBuyCost", args: [mid] })) as bigint;
      } catch (e) {
        ...
        if (oogCount >= 2) {
          ...
          try {
            const oneCost = (await publicClient.readContract({ address: token, abi: ctArtifact.abi, functionName: "calculateBuyCost", args: [1n * 10n ** 18n] })) as bigint;
            if (oneCost <= ethBudget) return { tokenAmount: 1n * 10n ** 18n, cost: oneCost };
  ...
  async function solveSellTokenAmount(token: Address, ethTarget: bigint, maxAvailable: bigint): Promise<{ tokenAmount: bigint; proceed: bigint } | null> {
    if (maxAvailable === 0n) return null;
    let lo = 1n;
    let hi = maxAvailable;
    ...
    for (let i = 0; i < 32; i++) {
      const mid = (lo + hi) / 2n;
      const proceed = (await publicClient.readContract({ address: token, abi: ctArtifact.abi, functionName: "calculateSellPrice", args: [mid] })) as bigint;
  ...
  for (const w of wallets) {
    const n = actionsFixed ?? (Math.floor(Math.random() * (actionsMax - actionsMin + 1)) + actionsMin);
  ...
  while ([...actionsPlan.values()].some((n) => n > 0)) {
    const active = wallets.map((w) => w.address).filter((a) => (actionsPlan.get(a) || 0) > 0);
    ...
    if (!token) { actionsPlan.set(addr, remaining - 1); await sleep(200); continue; }
    const isBuy = Math.random() < 0.5;
    const ethAmt = randBetween(minEth, maxEth);

    const tokenContract = getContract({ address: token, abi: ctArtifact.abi, client: { public: publicClient, wallet: wc } });

    let success = false;
    if (isBuy) {
      const solved = await solveBuyTokenAmount(token, ethAmt);
      if (!solved || solved.tokenAmount === 0n) {
        console.log(`[skip][buy] ${w.address} token=${token} reason=no-solution for ~${ethAmt} wei`);
      } else {
        console.log(`[buy] ${w.address} token=${token} eth~${ethAmt} sends=${solved.cost} for tokens=${solved.tokenAmount}`);
        if (!simulate) {
          const hash = await tokenContract.write.buyTokens([solved.tokenAmount], { account: w, chain, value: solved.cost });
      ...
      const bal = (await publicClient.readContract({ address: token, abi: ctArtifact.abi, functionName: "balanceOf", args: [w.address] })) as bigint;
      if (bal === 0n) {
        console.log(`[skip][sell] ${w.address} token=${token} reason=no-balance`);
      } else {
        const solved = await solveSellTokenAmount(token, ethAmt, bal);
        if (!solved || solved.tokenAmount === 0n) {
          console.log(`[skip][sell] ${w.address} token=${token} reason=no-solution for ~${ethAmt} wei; bal=${bal}`);
        } else {
          console.log(`[sell] ${w.address} token=${token} targetETH~${ethAmt} selling=${solved.tokenAmount} expect>=${solved.proceed}`);
          if (!simulate) {
            const hash = await tokenContract.write.sellTokens([solved.tokenAmount], { account: w, chain });
  ...
  for (const t of tokensAll) {
    try {
      const stats = (await publicClient.readContract({ address: t, abi: ctArtifact.abi, functionName: "getTokenStats", args: [] })) as readonly [bigint, bigint, bigint, bigint, bigint, bigint];
      const tradeStats = (await publicClient.readContract({ address: TradingEngineAddr, abi: teArtifact.abi, functionName: "getTokenTradingStats", args: [t] })) as readonly [bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint];
      const [totalSupply, totalSold, currentPrice, marketCap, holderCount, creatorFees] = stats;
      const [totalVolume, dailyVolume, weeklyVolume, totalTrades, uniqueTraders, averageTradeSize, priceChange24h, allTimeHigh, allTimeLow] = tradeStats;
      console.log(`- token ${t}`);
      console.log(`    supply=${totalSupply} sold=${totalSold} price=${currentPrice} mcap=${marketCap} holders=${holderCount} fees=${creatorFees}`);
      console.log(`    vol24h=${dailyVolume} vol7d=${weeklyVolume} totalVol=${totalVolume} trades=${totalTrades} traders=${uniqueTraders}`);
      console.log(`    avgTrade=${averageTradeSize} price24h=${priceChange24h} ath=${allTimeHigh} atl=${allTimeLow}`);
    } catch (e) {
  ...
  for (const w of wallets) {
    ...
    for (const t of tokensAll) {
      const bal = (await publicClient.readContract({ address: t, abi: ctArtifact.abi, functionName: "balanceOf", args: [w.address] })) as bigint;
      if (bal > 0n) console.log(`    token ${t} balance=${bal}`);
    }
  }
