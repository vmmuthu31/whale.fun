...
async function main() {
  ...
  const tokenFactory = await deployContract("X402TokenFactory", [account.address]);
  
  // Step 3: Deploy X402TokenGraduation
  const tokenGraduation = await deployContract("X402TokenGraduation", [account.address]);
  
  // Step 4: Deploy X402TradingEngine
  const tradingEngine = await deployContract("X402TradingEngine", [account.address]);
  
  // Step 5: Deploy X402DashboardDataProvider
  const dashboardDataProvider = await deployContract("X402DashboardDataProvider", []);
  ...
  try {
    ...
    console.log("ℹ️  X402TokenGraduation doesn't have an initialize function or it's already set");
  }
