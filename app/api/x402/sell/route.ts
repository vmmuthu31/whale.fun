...
async function handleSellRequest(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { walletAddress, amount, authorization, chainId, tokenAddress } =
      body;
    ...
    const facilitatorBalanceNum = parseFloat(
      ethers.formatUnits(facilitatorBalance, decimals)
    );
    ...
    if (authorization) {
      ...
      try {
        ...
        try {
          domainSeparator = await tokenContract
            .DOMAIN_SEPARATOR()
            .catch(() => null);
          transferTypeHash = await tokenContract
          ...
          console.log(
            ...
            transferTypeHash || "Not available"
          );
        } catch (e) {
          console.log(
            "Could not fetch EIP-712 domain info, proceeding with standard transferWithAuthorization"
          );
        }
        ...
        try {
          ...
          console.log(
            "Message:",
            JSON.stringify(signatureData.message, null, 2)
          );
          console.log("Types:", JSON.stringify(signatureData.types, null, 2));

          console.log(
            "Attempting transferWithAuthorization with separate v, r, s parameters"
          );
          ...
          console.error(
            "Standard transferWithAuthorization failed, error details:",
            {
              error: error.message,
              code: error.code,
              data: error.data,
          ...
          try {
            console.log(
              "Trying alternative signature format with packed signature..."
            );
            ...
            console.log(
              "Transaction sent with packed signature, waiting for confirmation..."
            );
            const receipt = await tx.wait();
            console.log(
              "Transfer successful with packed signature, tx hash:",
            ...
            console.error(
              "Alternative transferWithAuthorization with packed signature also failed:",
              {
            ...
            try {
              console.log(
                "Trying one more time with different signature format..."
              );
              ...
              console.log(
                "Transaction sent with object signature, waiting for confirmation..."
              );
              const receipt = await tx.wait();
              console.log(
                "Transfer successful with object signature, tx hash:",
                receipt.transactionHash
              );
              ...
              console.error(
                "Final attempt with object signature also failed:",
                {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Authorization required. User must sign EIP-3009 authorization for token transfer.",
